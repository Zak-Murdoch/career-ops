...
**For each company in `tracked_companies` with a `parser:` configured:** run the local parser defined in `portals.yml`. This level is ideal when the careers page uses SSR or stable HTML and a JavaScript, Python, or other local-runtime script already exists to extract jobs without agent involvement.
...
The parser is typically company-specific and already knows the URL, selectors, and pagination. `args` is optional: use it to help whoever built the script — for example, to reuse it across companies, pass `{careers_url}` or `{company}`, enable a debug flag, save a JSON snapshot, or control any parser-specific behavior.
...
The scanner does not need to retain the full JSON after reading stdout. If a parser also generates an artifact for auditing or debugging, save it to `data/parser-output/{company}/` and keep it out of git (JSON files in `.gitignore`; `.gitkeep` files stay in git to preserve the directory structure).

### Rule: successful local parser — no repeat expensive scraping

The purpose of `scan_method: local_parser` is to **reduce tokens**: prevent the LLM from re-scraping the same company with Playwright or redundant APIs.

During the agent scan, keep in memory the **`local_parser_ok`** set: company names (`tracked_companies[].name`) where Level 0 completed successfully:
...
|-------|----------------------------------------|
| **1 — Playwright** | **Skip** — no `browser_navigate` to its `careers_url` (most expensive method in tokens) |
...
- Parser **failed** → the company does **not** enter `local_parser_ok`; Levels 1 and 2 apply normally (same criterion as the `scan.mjs` fallback when a parser fails and an ATS API exists).
- Level 3: do not disable cross-cutting queries (`site:jobs.ashbyhq.com`, `site:boards.greenhouse.io`, etc.) — they serve to discover **new** companies. Only filter results from companies already in `tracked_companies` with a successful parser.
...
**Level 0 recommended:** run `node scan.mjs` (or `npm run scan`) at the start of the agent workflow. This covers local parsers + APIs in a single zero-token step and returns which companies used `local-parser` successfully.

### Level 1 — Direct Playwright (PRIMARY)

**For each company in `tracked_companies` not in `local_parser_ok`:** navigate to its `careers_url` with Playwright (`browser_navigate` + `browser_snapshot`), read ALL visible job listings, and extract the title + URL of each one. This is the most reliable method because:
...
The `search_queries` with `site:` filters cover portals cross-sectionally (all Ashby, all Greenhouse, etc.). Useful for discovering NEW companies not yet in `tracked_companies`, but results may be stale. After filtering hits from companies in `local_parser_ok`, the remaining results are deduplicated against Levels 0–2.
...
Levels are additive — executed in order, results merged and deduplicated. Companies in `local_parser_ok` **do not** go through Levels 1 or 2; at Level 3 they only contribute cross-cutting discovery (other companies on the same portal).
...
5. **Level 2 — ATS APIs / feeds** (parallel):
   ...
   d. For **BambooHR**, the list only returns basic metadata. For each relevant item, read `id`, GET `https://{company}.bamboohr.com/careers/{id}/detail`, and extract the full JD from `result.jobOpening`. Use `jobOpeningShareUrl` as the public URL if provided; otherwise use the detail URL.
...
6. **Level 3 — WebSearch queries** (parallel if possible):
   ...
   d. Accumulate the rest in candidates list (dedup with Levels 0+1+2)
